...
**`loan_amnt`** : Jumlah pinjaman yang diajukan oleh peminjam (Memiliki distribusi cenderung ***Right Skewd***)<br>

**`funded_amnt`** : Jumlah total yang berkomitmen untuk pinjaman tersebut pada saat itu. (Memiliki distribusi cenderung ***Right Skewd***)<br>

**`int_rate`** : Tingkat bunga yang dikenakan pada pinjaman. (Memiliki distribusi cenderung ***Normal***)<br>

**`annual_inc`** : Pendapatan tahunan yang dilaporkan sendiri oleh peminjam saat pendaftaran. (Memilki distribusi cenderung ***Right Skewd***)<br>

**`dti`** : Rasio yang dihitung dengan menggunakan total pembayaran utang bulanan peminjam terhadap total kewajiban utang (Memilki distribusi***Normal***)<br>
...
#### ***Note*** : Sebelumnya sudah dilakukan penghapusan outliers, namun performa model tidak cukup bagus, sehingga tidak jadi dilakukan penghapusan outliers.
...
```sh
drop_columns2 = ['season','yearly_loan','acc_now_delinq','verification_status','grade','tot_coll_amt','emp_length','home_ownership','funded_amnt','initial_list_status','application_type']
```
...
roc_auc (crossval test): 0.7943803650359731
```
...
roc_auc (test-proba): 0.75
roc_auc (train-proba): 0.75
roc_auc (crossval train): 0.9999999999323446
roc_auc (crossval test): 0.7947645713929599
```
<br>
<br>
<br>
...
roc_auc (crossval train): 0.9999999999715137
roc_auc (crossval test): 0.7945165771460126
```
...
roc_auc (crossval train): 0.9999999998896149
roc_auc (crossval test): 0.7944862446565273
```
<br>
<br>
<br>
...
roc_auc (crossval train): 0.9999999999430269
roc_auc (crossval test): 0.7945113041968569
```
...
roc_auc (crossval train): 0.9999999999359053
roc_auc (crossval test): 0.7947332645171097
```
<br>
<br>
<br>
...
roc_auc (train-proba): 1.00
roc_auc (crossval train): 0.9999999999928784
roc_auc (crossval test): 0.7946709586603351
```
...
roc_auc (crossval train): 0.9999999999964391
roc_auc (crossval test): 0.7946212344116157
```
<br>
<br>
<br>
...
roc_auc (crossval train): 0.9999999999857568
roc_auc (crossval test): 0.7945241703535844
```
...
roc_auc (crossval train): 1.0
roc_auc (crossval test): 0.7946053118597496
```
<br>
<br>
<br>
...
roc_auc (test-proba): 0.82
roc_auc (train-proba): 0.83
roc_auc (crossval train): 0.9999999999359055
...
roc_auc (crossval train): 0.9999999999430271
roc_auc (crossval test): 0.7945878922722249
```
<br>
<br>
<br>
...
roc_auc (test-proba): 0.81
roc_auc (train-proba): 0.81
roc_auc (crossval train): 0.9999999999964393
roc_auc (crossval test): 0.7948196897875367
```
...
roc_auc (test-proba): 0.82
roc_auc (train-proba): 0.84
roc_auc (crossval train): 0.999999999964392
roc_auc (crossval test): 0.7946654209683093
```
<br>
<br>
<br>
...
#### Model yang dipilih adalah model `LightGBM` yang belum di tuning parameternya. Metrix utama yang digunakan adalah `Precision` yang dimana kita tidak ingin **False Positive** tinggi atau mengurangi **False Positive** sekecil-kecilnya.<br>

#### Dalam konteks lending company, **False Positive** berarti menyetujui pinjaman yang seharusnya ditolak, False Positive dapat menyebabkan kerugian finansial bagi perusahaan.
<br>
<br>
<br>
...
Model ini memiliki score precision yang tinggi yakni mencapai **0.82** dengan probabilitas machine learning sebesar **0.82**. Model tidak overfit maupun underfit yang dapat disebut sebagai model ***best fit***. <br>
...
#### - Menawarkan insentif atau diskon bagi pelanggan yang memilih pinjaman dengan durasi 60 bulan, seperti bunga yang lebih rendah, biaya administrasi yang lebih murah, atau cicilan yang lebih fleksibel. Hal ini dapat meningkatkan permintaan dan pendapatan dari pinjaman 60 bulan.

#### - Menetapkan kriteria kelayakan yang lebih ketat untuk pelanggan yang memiliki pendapatan tahunan di bawah 70000 dan ingin meminjam untuk 60 bulan, seperti skor kredit yang lebih tinggi, jaminan yang lebih besar, atau persyaratan dokumen yang lebih lengkap.

#### - Meningkatkan kualitas layanan dan hubungan pelanggan dengan pelanggan yang meminjam untuk 60 bulan, terutama yang memiliki pendapatan tinggi, karena mereka merupakan segmen yang potensial dan loyal
